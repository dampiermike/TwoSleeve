...
"""
═══════════════════════════════════════════════════════════════════════════════
  TwoSleeves Optimized v1.1  —  Reference Implementation
  Per spec: TwoSleeves_Optimized_Build_Guide_v1_1.md

  CHANGE FROM v1: the CASH state holds BIL (1-3 month T-bills) instead of
  0%-yield dollars. A sleeve only reaches the cash state after a defensive
  stop (a confirmed bear); parking that capital in T-bills earns the
  risk-free rate during long crisis stretches. This is also the more
  realistic model — brokerages sweep idle cash into money-market funds.
...
DATA DEPENDENCY (zero external APIs, no VectorVest, pure JSON files):
  ...
  VIX is NOT required for v1.1 (no pyramid sizing). Future v2 with pyramid
...
OUTPUTS (written to this directory):
  backtest_two_sleeves_v1_1_equity_curve.csv      — daily portfolio + per-sleeve
  backtest_two_sleeves_v1_1_trades.csv            — entry/exit trade log
  backtest_two_sleeves_v1_1_rebalance_events.csv  — annual rebalance detail
"""
...
print()
print("=" * 80)
print("  TWO-SLEEVES OPTIMIZED v1.1  —  Per-Sleeve WMA/SMA  ·  Cash → BIL")
print(f"  Equity sleeves : ${EQ_ALLOC_EACH:,.0f} each × 2  (90%)")
print(f"  Safety sleeve  : ${SAFETY_INIT:,.0f}        (10% GLD, buy & hold)")
print(f"  Cash state     : holds {CASH_TICKER} (T-bills) instead of 0%-yield cash")
...
def make_sleeve(signal, vehicle, defensive, wma_period, sma_period, init_equity):
    return dict(
        ...
        c_shares=0.0,                      # BIL shares held while in cash state
...
# Both sleeves start in the cash state — convert initial $ into BIL at adj[0]
...
for i in range(n):
    ...
    for sl in eq_sleeves:
        ...
        # Liquidate current position → sl["cash"] (transient dollar bridge)
        if sl["state"] == "vehicle":
            ...
            # Liquidate the BIL holding (cash periods are not logged as trades)
            sl["cash"] = sl["c_shares"] * co; sl["c_shares"] = 0.0

        # Deploy sl["cash"] into the next state
    ...
    for sl in eq_sleeves:
        ...
        else:  # cash state — value the BIL holding
    ...
    if cur_year > prev_year:
        ...
        for sl in eq_sleeves:
            ...
            if sl["state"] == "vehicle":
                ...
                sl["equity"]   = eq_target
            else:  # cash state — resize the BIL holding
...
# ── Save CSVs ─────────────────────────────────────────────────────────────────
eq_csv  = OUT_DIR / "backtest_two_sleeves_v1_1_equity_curve.csv"
tr_csv  = OUT_DIR / "backtest_two_sleeves_v1_1_trades.csv"
reb_csv = OUT_DIR / "backtest_two_sleeves_v1_1_rebalance_events.csv"
...
print()
print("=" * 80)
print("  RESULT  vs  SPEC TARGETS (TwoSleeves_Optimized_Build_Guide_v1_1.md)")
print("=" * 80)
TARGETS = dict(final_eq=50_081_555, cagr=26.76, max_dd=-37.46, sharpe=0.8752,
               trades=86, bars=6594)
